...
func open_ui(ui_name:String,data = null)->UIBase:
	if not _ui_root:
		Debug.Error(name,"ui根节点为空")
		return null
	if is_ui_opened(ui_name):
		Debug.Error(name,"%s已经打开" % ui_name)
		return null
	var path = _ui_path % ui_name
	if not ResourceLoader.exists(path,"PackedScene"):
		Debug.Error(name,"不存在路径为%s的ui场景" % path)
		return null
	var ui_scene:PackedScene = load(path)
	if not ui_scene:
		Debug.Error(name,"%s加载失败" % ui_name)
		return null
	var ui_node:UIBase = ui_scene.instance()
	if not ui_node:
		Debug.Error(name,"%s实例化失败" % ui_name)
		return null
	if not ui_node.has_method("on_open"):
		Debug.Error(name,"%s没有on_open方法" % ui_name)
		return null
	Debug.Log(name,'打开%s' % ui_name)
...
func show_toast(toast_name:String,msg:String)->void:
	...
	if not ResourceLoader.exists(path,"PackedScene"):
		Debug.Error(name,"不存在路径为%s的toast" % path)
		return
	var toast_scene:PackedScene = load(path) as PackedScene
	if not toast_scene:
		Debug.Error(name,"%s加载失败" % toast_name)
		return
	var toast_node = toast_scene.instance()
	if not toast_node:
		Debug.Error(name,"%s实例化失败" % toast_name)
		return
