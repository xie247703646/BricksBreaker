...
public class PolicyActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        TextView policy_content = findViewById(R.id.policy_content);

        String game_name = "BricksBreaker";
        SharedPreferences preference = this.getSharedPreferences(game_name, Context.MODE_PRIVATE);
        ...
        cbAgree.setOnClickListener(view -> {
            btnYes.setEnabled(cbAgree.isChecked());
        });
        ...
        String str = "感谢您信任并选择我们的游戏《game_name》\n" +
                ...
                "我们会积极按照国家防沉迷政策要求，通过启用防沉迷系统保护未成年人的合法权益。我们会通过易玩（上海）网络科技有限公司提供的TDS 实名认证和防沉迷服务进行校验判断相关账号的实名信息是否为未成年人，进而决定是否将此账号纳入到防沉迷体系中。另外，我们会收集您的登录时间、游戏时长信息，通过从系统层面自动干预和限制未成年人游戏时间、启用强制下线功能方式，引导未成年人合理游戏，并在疑似未成年人消费后尝试联系其监护人进行提醒、确认与处理，帮助未成年人健康上网。";

        str = str.replaceAll("policy_url",policy_url);
        str = str.replaceAll("game_name",game_name);

        policy_content.setText(str);
        policy_content.setText(Html.fromHtml(str));
        policy_content.setMovementMethod(LinkMovementMethod.getInstance());
    }
